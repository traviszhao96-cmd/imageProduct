...
- `first_zoom_ratio`
- `last_zoom_ratio`
- `first_orientation` / `last_orientation`: 手机方向状态，`0` = 竖屏 (vertical)，`1` = 横屏 (horizontal)
  - ⚠️ 注意：本地解析后的数据中，orientation 字段已经是映射后的值（0/1），不是原始 Android 传感器值（0/90/180/270）。查询时直接使用 0=竖屏, 1=横屏 即可。
- `first_lux` / `last_lux`
- `first_adrc` / `last_adrc`
- `first_cct` / `last_cct`
- `first_face_count` / `last_face_count`
