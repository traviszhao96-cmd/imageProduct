...
- Local DBs follow naming convention `{models}_{region}_{YYYYMMDD-MMDD}.db`:
  - `4a_4aPro_ROW_20260323-0329.db` — ROW, 3/23-3/29, ~30万张
  - `4a_4aPro_India_20260415-0421.db` — India, 4/15-4/21, ~559万张
...
- **If the user does NOT specify which dataset to use, always list available DBs and let them choose before querying**
- Present choices with key metadata: date range, region, models, data scale
...
- `orientation` (photo_info): `0` = 竖屏 (vertical), `1` = 横屏 (horizontal)
- `first_orientation` / `last_orientation` (video_info): `0` = 竖屏 (vertical), `1` = 横屏 (horizontal)
  - ⚠️ 注意:本地解析后的数据中,orientation 字段已经是映射后的值(0/1),不是原始 Android 传感器值(0/90/180/270)。查询时直接使用 0=竖屏, 1=横屏 即可,不需要再按角度范围判断。
...
- If the user does not specify a dataset, list available DBs with metadata and let them choose.
