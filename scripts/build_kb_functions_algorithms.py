...
ROWS = [
    ...
    row(
        ...
        "预览框",
        "高倍率导航框",
        "高倍率变焦时显示全局视野和当前取景位置，帮助用户在画面大幅放大后定位主体、找回目标并稳定构图。它是预览交互能力，不等同于长焦摄像头。",
        "当前项目达到高倍率导航框的触发倍率，并且由独立长焦摄像头或 200MP 主摄的 ISZ、Hex Zoom、裁切超分等高倍率链路提供有效画面时填写支持。不得仅因项目没有长焦摄像头就判定不支持；需综合确认项目最大变焦倍率、长焦选型和 200MP 主摄高倍率能力。",
        "依赖项目最大变焦倍率、导航框触发阈值、长焦选型或 200MP 主摄高倍率链路，以及全局视野画面、当前取景区域映射和预览 UI。",
        "逐个进入项目定义的高倍率焦段，确认导航框在阈值处出现、全局视野与主预览方向一致、当前取景区域位置准确；缩放和移动构图时检查更新时延、边界、遮挡与退出行为。",
        "旧称“长焦预览框”；统一更名为“高倍率导航框”，避免把支持条件错误绑定到 Tele 硬件。",
        app_binding="当前 CameraApp 代码基线未检出明确 UI node / config key，待 APP 补充",
        implementation_status="待确认",
    ),
    row(
        "照片 / 人像 / 夜景 / 高像素",
        "算法",
    ...
    row(
        ...
        "照片模式的右侧暂态控制开关。低照且夜景算法效果优于普通拍照时出现，用户未关闭时按 Super Night 链路拍照；它只是照片模式的控制入口，不代表超级夜景算法本身，也不能用开关是否出现判断人像模式是否支持夜景。",
        "照片模式具备低照检测、夜景收益判断、Super Night 链路和对应暂态开关时填写支持。人像模式可在没有该开关的情况下由场景检测自动进入夜景组合链路，二者需分别验收。",
    ...
    row(
        ...
        "后处理算法",
        "超级夜景+美颜",
        "面向低照人脸拍摄的组合后处理能力，在超级夜景提升主体与背景亮度、噪声和细节的同时应用虚化、人脸检测、美颜、滤镜与 FRT，使暗光人像保持自然肤色和纹理，并降低多帧合成与美颜叠加造成的鬼影、塑料感或边缘异常。人像模式可由场景检测自动进入该链路，前端不要求提供可交互的自动夜景暂态开关。",
        "当前输出摄像头同时接入人像虚化、Super Night、人脸检测、美颜、滤镜与 FRT 链路，并允许低照人脸策略叠加时填写支持；不能因没有自动夜景开关而判定不支持。人像链路内部使用 UW 辅助 depth 不代表 UW 可独立出图，项目 FL 的 UW 列仍应按用户可选输出摄像头判断。",
        "依赖 Super Night、多帧对齐、人脸检测、美颜算法、低照人脸策略，以及人物运动和多人场景下的融合稳定性。",
        "在人像模式的不同低照等级覆盖单人、多人、运动人物和画面边缘人脸；无需查找暂态开关，结合场景检测结果与算法 tag 确认虚化、夜景、美颜、滤镜和 FRT 同时生效，并检查肤色、纹理、噪声、鬼影、虚化/发丝边缘和处理耗时。",
    ),
...
STRUCTURAL_CHILD_ROWS = [
    ...
    row("照片 / 人像 / 视频 / 夜景 / 高像素 / 专业", "功能", "AE/AF", "EV 曝光补偿",
        "在自动曝光基准上施加用户曝光补偿；它是需要按模式单独开放的用户控制，不能由底层 AE 能力或照片模式的自动夜景处理推导为支持。", "仅当当前模式的产品范围明确暴露 EV 控制且 HAL compensation range 非零时支持；独立夜景模式必须单独确认入口，照片模式可自动调用夜景处理不代表夜景模式需要新增 EV 调节。", "依赖模式产品范围、AE compensation range/step、滑杆 UI 和状态记忆。", "支持时验证最小、0、最大 EV 的预览、Capture Result 与成片；不支持的模式确认无 EV 入口且不会产生可恢复的 EV 状态。",
...
NODE_ID_OVERRIDES = {
    ...
    "高倍率导航框": "kb.preview.high_zoom_navigation",
