...
ROWS = [
    ...
    row(
        "照片 / 人像 / 夜景 / 高像素",
        "算法",
        "自然质感人像 / Natural Texture Portrait",
        "FRT / 人像清晰度提升",
        "自然质感人像能力族中的独立清晰度增强算法。FRT（Face Restoration Technology）在人脸检测成立时恢复和增强人脸细节，重点改善人脸区域清晰度；它不是美颜参数或肤质修饰功能。",
    ...
    row(
        ...
        "自然质感人像 / Natural Texture Portrait",
        "美颜升级 / Beauty Upgrade",
        "自然质感人像能力族中的独立美颜效果升级，仅用于照片和人像模式的前置摄像头。升级包括现有磨皮、美白、亮眼和胡须保护的参数与效果优化，并新增匀肤、肤色分层、性别分层、年龄分层和脸型流畅能力；目标是在保留真实肤色、纹理、毛发和个人特征的前提下提升干净度与自然度。",
    ...
    row(
        ...
        "取帧策略",
        "PZL",
        "拍照取帧策略：按下快门后再开始获取用于成片的帧，区别于 ZSL（Zero Shutter Lag）从快门前的预缓存中选择帧。PZL 本身不是电子防抖算法。",
        "当前模式、镜头和场景明确使用快门后取帧链路时填写支持；不能依据 Photo EIS 是否支持来推断。",
        "依赖相机取帧时序、曝光与多帧算法链路、快门响应策略和当前模式配置。",
        "结合算法 tag 或日志确认按下快门后的取帧起点、帧数和时序，并检查快门延迟、运动拖影及成片效果。",
    ),
    row(
        "视频 / 慢动作 / 延时摄影 / 前后双录",
        "算法",
    ...
    row(
        ...
        "实时算法",
        "Video HDR 算法",
        "视频录制时通过 Sensor HDR 曝光/读出模式与 ISP/算法处理扩展动态范围，保留高光和暗部细节，并按支持的 HDR 格式编码输出。当前项目范围：26111 不支持；26121 支持 Main/Tele，不支持 UW/Front。需逐摄像头确认分辨率/帧率、Sensor mode、输出格式，以及与 EIS、变焦、风格/LUT、Log 的兼容关系和功耗温升。",
    ...
    row(
        ...
        "按模式、摄像头和亮度区间确认 MFNR 是否生效，并记录与 HDR、Super Night、Motion Capture 的互斥或切换条件；仅存在多帧库不等于当前场景进入 MFNR。",
    ...
    row(
        ...
        "当前模式和摄像头启用人脸检测/关键点，并在人脸位于大视场角边缘时调用局部人脸形变矫正链路，才填写支持。",
    ...
    row(
        ...
        "Toolbar",
        "Flash",
        "夜景模式的补光入口能力。当前 26111/26121 夜景模式均不开放 Flash：Main/UW/Tele/Front 全部不支持，后置 LED Flash/Torch 和前置屏幕补光均不进入夜景拍摄链路。夜景依赖环境光下的长曝光与多帧合成，避免补光改变场景光照并与夜景曝光策略冲突。",
        "仅当夜景模式明确提供独立补光入口，并允许对应摄像头在夜景算法链路中使用 LED Flash、Torch 或屏幕补光时填写支持；不得沿用照片模式的 Flash 支持结论。",
        "依赖夜景模式 Toolbar 配置、Super Night 曝光/多帧策略、LED Flash/屏幕补光控制及模式状态隔离。",
        "进入夜景模式确认 Flash 入口隐藏或不可用；从照片模式携带不同 Flash 状态切入夜景，确认状态不继承且拍摄过程不会触发 LED、Torch 或屏幕补光。",
    ),
    row(
        "视频 / 慢动作 / 延时摄影 / 前后双录",
        "功能",
    ...
    row("夜景 / 人像", "算法", "后处理算法", "超级夜景+美颜", "夜景成像链路中叠加人脸检测与美颜处理。", "当前模式和摄像头同时支持 Super Night、FD 和美颜，并允许低照人脸策略叠加时填写。", "依赖 Super Night、人脸检测、美颜算法、低照人脸策略。", "低照人脸场景开启美颜拍摄，确认夜景和美颜同时生效且自然。"),
