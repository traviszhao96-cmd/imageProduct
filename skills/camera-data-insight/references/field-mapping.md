...
- The NPS `device_id` is the Android ID used by telemetry as `aid`. In Athena it is stored in the `items` array, not as a top-level column: `element_at(filter(items, x -> lower(x.key) = 'aid'), 1).string_value`.
- A verified India sample matched this `items.aid` value and returned both general behavior events and `NTCamera` events. Query NPS users by `items.aid`; do not query `user_pseudo_id`, `device.device_id`, `event_params.aid`, or `user_properties.aid` for this mapping.
...
- For the verified India sample, neither the raw 16-character NPS `device_id` nor its ordinary MD5 value matched Athena `user_pseudo_id`, `user_id`, `device.device_id`, or `device.device_id_2` during the activation-to-push window. This was expected because the correct field is `items.aid`; no 16→32 conversion is required.
- Use `/push/nps/survey/lookup` (after its real database lookup is implemented) for `refid → NPS device_id`, then use that value directly as Athena `items.aid`.
