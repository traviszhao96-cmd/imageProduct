<title>【PRD】Camera 5.1 - 200MP 高像素</title>

# 前言

<callout emoji="💡">
在空行输入“/高亮块” 插入高亮块，突出显示重点信息
</callout>

# 一、 版本信息

<grid>
<column width-ratio="0.333333">
<callout emoji="⏰">
版本号：1.1
</callout>
</column>
<column width-ratio="0.333333">
<callout emoji="📆">
20251023
</callout>
</column>
<column width-ratio="0.333333">
<callout emoji="👮">
审核人
</callout>
</column>
</grid>



# 二、 变更日志

| **时间** | **版本号** | **变更人** | **主要变更内容** |
|-|-|-|-|
| 2025/10/23 | 1.0 | Lia | 创建文档 |
|  |  |  |  |
|  |  |  |  |



# 三、 需求背景

## 产品 / 数据现状

1. 26111/26121 首次采用**200MP（HP5）**主摄，具备输出200MP高像素照片的能力，我们需要结合技术实现和用户拍摄需求制定高像素模式的功能交互
2. Ksp文档：
3. 可行性文档：<cite doc-id="XwYUwdXlQikqznkwtChlSgc8gec" file-type="wiki" title="25131  Blastoise Pro Camera KSP" type="doc"></cite>
4. 线上机型高像素模式数据分析<cite type="user" user-id="ou_1e068f80b2831f5bc95787032143a546" user-name="Travis Zhao"></cite>

<sheet sheet-id="yJjWK1" token="Whg7s1cA5hlFTNtU9TkllyW4gsb"></sheet>



## 竞品分析

> 列出竞品对比的主要信息和关键结论，可输入 @ 在此附上详细的竞品分析报告并添加在【附录】中



<table><colgroup><col/><col/><col/><col/><col/><col/><col/><col/></colgroup><tbody><tr><td>机型</td><td>摄像头</td><td>高像素-入口</td><td>记忆状态</td><td>快门体验</td><td>后处理</td><td>相册管理</td><td>特性功能</td></tr><tr><td>x300 pro</td><td>长焦</td><td><img name="1.jpeg" href="https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/authcode/?code=ZDdiYzk1ZjEwM2ZhNzgyMTJmNDZkMWExZjQ0ZTNmZTVfZWY1ZDc1ODJkYTE3YjU3NWQ4NGFmYTdiMGU5Nzk0YjdfSUQ6NzY0Nzc5MjUzNDMzNjU3MzE1MV8xNzgzNDA2MzE5OjE3ODM0MDk5MTlfVjM" mime="image/jpeg" scale="0.675926" src="HCYYbewqAo0mMjxaXikl3yvMgMe"/></td><td>杀进程后恢复默认</td><td>快门动画3s+<figure view-type="Preview"><source href="https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/authcode/?code=ZDA0MjlmNmRlZDNlMGU4N2RlNDg1NDkwYTI5NzY2OWZfMjE1ODRmNmQ3ZTViN2Y4OTBiN2E5MjlhNDY0ZDE0MDBfSUQ6NzY0Nzc5MjUzNTgyMTI0MjA4MF8xNzgzNDA2MzE5OjE3ODM0MDk5MTlfVjM" mime="video/mp4" origin-height="960.000000" origin-width="540.000000" token="T3SibsbVeo7fdPxxQGklU3oZgcd"/></figure></td><td>/</td><td>有单独的高像素相册<img name="image.png" href="https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/authcode/?code=NGI1M2Y1YjUwNTEzYTBjNjdmNmYyZmUwYmE0ZTZhZWRfMmZiYWNiYjc0NmQ1OTRiNzk2NzViMzFhMTI4OTQyOGRfSUQ6NzY0Nzc5MjUzNDAwOTQzMzgyM18xNzgzNDA2MzE5OjE3ODM0MDk5MTlfVjM" mime="image/png" scale="1.000000" src="VZiAbu8VeoAJxOxQX0jluhCogZc"/></td><td>支持人像2亿、支持电影分镜自动裁图拼图<img name="image.png" href="https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/authcode/?code=NGQ2MTYwYzQxMzAxMDdmYjg4YjcyYzE5Y2IwZTUyNThfYWY5NTg3YTQ2ZTI5NjEwNWIzYjQ2OTA1YjFmYmE0ZDlfSUQ6NzY0Nzc5MjUzNDA0ODgyMDk2Ml8xNzgzNDA2MzE5OjE3ODM0MDk5MTlfVjM" mime="image/png" scale="0.109687" src="XbXpbtsO7oVcAsx1PgHlF77agvb"/></td></tr><tr><td>Findx9 pro</td><td>长焦</td><td></td><td></td><td></td><td></td><td></td><td></td></tr><tr><td>vivo6e</td><td>主摄</td><td></td><td></td><td></td><td></td><td></td><td></td></tr><tr><td>其他有2亿摄像头的机型</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td></tr></tbody></table>

# 四、 需求范围

> 可条理性地罗列需求范围或信息架构

1. 项目范围：对硬件。首上项项目为26111/26121，后续200MP项目默认继承
2. 老项目回落：不支持回落，具体回落计划根据回落排期确

## 需求列表&需求单

<table><colgroup><col/><col/></colgroup><tbody><tr><td>需求名称</td><td>需求描述</td></tr><tr><td>200MP高像素</td><td><ol><li seq="1">首次进入相机，对200MP的入口、使用体验做个说明--看下其他手机的体验</li><li>开启200MP后，顶部栏有入口，支持点击关闭和开启</li><li>拍摄预计有3s的处理时长，过程中不可再次点击快门，需要有快门动画，建议做的有风格一些，提升用户的耐心；以及有处理中文案提醒</li><li>专业模式支持高像素拍摄</li><li>打开的记忆状态规则</li><li>功能兼容状态-- follow 25111 50MP高像素</li><li>算法兼容状态待明确</li></ol></td></tr></tbody></table>

# 五、 功能详细说明

## 产品流程图（略）

> 将鼠标悬浮至下方空白图形模块，点击**编辑**，即可进入流程图创作你的产品流程图

## 交互原型图

> 在空白行输入“/Figma” ，插入 Figma 设计稿
> 
> 或直接粘贴设计稿地址至文档，并展示为“内嵌网页”

## 功能说明

1. 模式和摄像头支持高像素情况

| 像素 | 超广 | 主摄 | 前置 | 长焦 |
|-|-|-|-|-|
| 是否有切换像素入口 | × | ✓，后置拍照、专业 | × | × |
| 12MP | ✓-默认 | ✓-默认，后置拍照、专业 | × | ✓-默认 |
| 16MP | × | × | ✓-默认 | × |
| 50MP | × | ✓，后置拍照、专业 | × | ✓ |
| 200MP | × | ✓，后置拍照、专业 | × | × |

1. 50MP、200MP 功能兼容情况

<table><colgroup><col/><col/><col/><col/><col/></colgroup><tbody><tr><td></td><td>像素 50MP/200MP</td><td>兼容情况</td><td>交互</td><td>技术评估</td></tr><tr><td rowspan="8">后置普通照片模式</td><td>HDR</td><td>✓</td><td>无特殊交互，同25111</td><td>符合HDR场景走HDR,插帧到高像素</td></tr><tr><td>夜景</td><td>✓</td><td>无特殊交互，同25111</td><td>符合夜景场景走夜景算法,插帧到高像素</td></tr><tr><td>滤镜</td><td>✓</td><td>无特殊交互，同25111</td><td></td></tr><tr><td>tuning</td><td>✓</td><td>无特殊交互，同25111</td><td></td></tr><tr><td>变焦</td><td>×</td><td>不支持变焦，变焦控件只有支持该像素的光变点，双指缩放时提示不可变焦，follow 25111</td><td></td></tr><tr><td>Motion Photo</td><td>×</td><td>互斥交互follow 25111</td><td></td></tr><tr><td>水印</td><td>✓</td><td>无特殊交互<br/>vivo200亿支持边框等类型的水印</td><td>内存/性能风险</td></tr><tr><td>preset</td><td>✓</td><td>在创建preset的时候，像素支持选择12\50\200MP</td><td></td></tr><tr><td rowspan="2">专业模式</td><td>参数</td><td>✓</td><td>相较于25111是新增的</td><td></td></tr><tr><td>raw</td><td>×</td><td>相较于25111是新增的</td><td>需要评估文件大小、拍摄时长</td></tr></tbody></table>

1. 交互

   1. 首次进入相机，对**200MP**的入口、使用体验做个说明，管理拍摄时长预期

   ![](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/authcode/?code=NDNkOTM0OTdjMGIwNmVjNGYyYTMzMTVmNzJjOGUzN2JfZmNjMjZiZDFhNjhkMDc5OTNkOTQwMjc3ZWJlMGY0NWZfSUQ6NzY0Nzc5MjUzNDQ4NzgxMzg1MV8xNzgzNDA2MzE5OjE3ODM0MDk5MTlfVjM)

   1. 功能入口、记忆状态---交互为展开

   ![](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/authcode/?code=YjU0YzJkNGE5NzY4YzRhNThlOTIwMzI5ZTljNzAyMjNfNjVjN2Y0ZGY4ZTZmNGQxOTA3ODFhNDAxZDUxNTFmZDZfSUQ6NzY0Nzc5MjUzNTM2MzI0Mzc0MF8xNzgzNDA2MzE5OjE3ODM0MDk5MTlfVjM)



1. 拍摄中交互体验问题：200MP的算法流程待决策：点击拍照，会有4-5s左右的等待时间；预览有一定时长的卡顿。基于可能的体验问题：

   1. 点击快门后，增加快门转圈动画，预览增加处理文案
   2. 拍照过程中，快门不再响应再次点击，；其他功能也不支持点击--
   3. 根据卡顿现象再看是否上预览的动画--待定

![](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/authcode/?code=ZmE3NGYxMTAyYWRlZDlmNDUwZDA5MjEwMWE1NDVmY2NfNmYwYWE1Y2IzYmVkOTZkZGI3Zjk2ZDJjMmJhNDUzZjZfSUQ6NzY0Nzc5MjUzNDI5NTEwNTI0Nl8xNzgzNDA2MzE5OjE3ODM0MDk5MTlfVjM)















## 需求词条--无需翻译

| 应用场景 | 中文词条 | 英文词条 |
|-|-|-|
| 首次弹窗介绍文案 | / |  |
| 拍摄过程中的文字提示 | 拍照处理中 |  |
| 200 MP 功能介绍-标题（可不翻译） | 200 MP | 200 MP |
| 200 MP 功能介绍-内容 |  | You can switch to 200MP mode to capture ultra-high-resolution photos. Please note there will be a short processing delay during shooting. For best quality, shoot in bright lighting conditions. |

# 七、 非功能需求

> 可以列举产品营销需求、运营需求、财务需求、法务需求、使用帮助、问题反馈等



# 八、 埋点

| **参数名** | **参数说明** | **参数值** |
|-|-|-|
| 50MP\200MP使用情况 |  |  |
| 高像素模式拍摄时走的算法 |  |  |
|  |  |  |
|  |  |  |
|  |  |  |
|  |  |  |
|  |  |  |
|  |  |  |
|  |  |  |



# 九、 项目规划

> 输入 @ 把正文提及的项目管理文档附在此处

<bitable table-id="tblc0KABKLhdlgKt" token="SSwBbFsHearrAxsMYfvlzjzxgce"></bitable>



# 附录

> 输入 @ 把正文提及的具体文档，或需求相关的其他说明文档附在此处以供查阅

<grid>
<column width-ratio="0.333333">
<callout emoji="📈">
数据分析报告
</callout>
- 此处插入数据分析报告
</column>
<column width-ratio="0.333333">
<callout>
用户调研报告
</callout>
- 此处插入用户调研报告
</column>
<column width-ratio="0.333333">
<callout emoji="🎨">
设计分析报告
</callout>
- 此处插入设计分析报告
</column>
</grid>



三.1 产品定义（2026.06.11 补充）

产品定位  
200MP 高像素模式的本质是用更高的空间分辨率满足"数毛"需求，是一个参数型卖点。面向的用户是追求极致清晰度和参数表现的群体，特别是印度市场对高像素参数有强烈偏好的专业用户。

与 50MP 的差异

- 50MP: 常规高像素，兼顾效果和速度
- 200MP: 清晰度更高的极端模式，但需要 3-5s 处理时间，取舍是速度
- 具体效果差异待项目实机验证

三档像素的用户心智模型  
12MP（默认日常）→ 50MP（高清晰度）→ 200MP（极致清晰，需等待）

交互逻辑

- 顶部状态栏: 循环切换 12 → 50 → 200 → 12
- 工具栏: 展开选择

专业模式支持 200MP 的定位

- 面向参数敏感的专业用户（特别是印度市场）
- 底层参数控制（RAW 支持待评估）
- RAW 主要满足专业用户后期处理需求

待决事项

- [TBD] 200MP vs 50MP 实际清晰度差异，需项目实机验证
- [TBD] RAW 文件大小和拍摄时长评估
