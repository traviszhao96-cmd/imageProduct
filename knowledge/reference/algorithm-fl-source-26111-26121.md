...
| 拍照 | 实时算法 | Photo EIS / PZL | 拍照高倍 zoom 稳定链路；PZL 是按下快门后再取帧的后取帧策略，区别于 ZSL。26111 主摄无 OIS，依赖 EIS/裁切空间。 | ✓ | [TBD] | ✗ | ✓ | [TBD] | ✓ | ✗ | 高倍手持拍摄；检查取景稳定、裁切和后取帧时序 |
| 视频 | 实时算法 | Video EIS | 录像默认普通防抖。26121 可与 OIS 叠加；26111 无 OIS。 | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | 1080P30 手持录制；确认稳定与视角裁切 |
| 拍照/视频/夜景/延时 | 实时算法 | SAT / 平滑镜头切换 | 多摄平滑切换。变焦跨过光学点时如果不支持 SAT，则需要硬切并通过模糊动画重启/切换摄像头。 | ✓ | ✓ | ✗ | ✓ | ✓ | ✓ | ✗ | 变焦跨镜头点；确认是 SAT 平滑切换还是硬切转场，并检查亮度/色彩/视角过渡 |
...
| 高像素 | 后处理算法 | HW Remosaic | 高像素模式基础链路。26111 提供 50MP / 200MP / 200MP Ultra；26121 提供 50MP / 50MP Ultra。Ultra 选项在 remosaic 后叠加 RAW HDR。 | ✓ | ✗ | ✗ | ✓ | ✗ | ✓ | ✗ | 高像素模式分别选择各规格出图；确认分辨率、RAW HDR/Ultra 标记、耗时和内存 |
| 拍照 | 后处理算法 | AI Zoom / AIGC SR | 26121 Tele 高倍 zoom 方案；26111 Base 不支持。 | ✗ | ✗ | ✗ | ✗ | ✗ | ✓ | ✗ | Tele 30x+ 场景；确认 AI Zoom / AIGC SR 入口和成片 |
