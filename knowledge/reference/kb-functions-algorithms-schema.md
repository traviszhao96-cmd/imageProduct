...
| 字段 | 含义 |
|---|---|
| 说明 | 单一结构化描述字段，必须按下述五段内容要求回答“是什么、为什么、达到什么目的” |
...
`说明`保持为一个字段，不再拆成多列。每个非目录知识节点必须依次包含：

1. `【能力定义】`：它是什么、在什么入口或链路中工作。
2. `【解决的问题】`：没有这项能力时，用户会遇到什么具体问题。
3. `【用户价值】`：用户能完成什么任务，或获得什么可感知收益。
4. `【产品目标】`：产品希望改善的结果，例如拍摄成功率、画质、效率、一致性或可控性。
5. `【能力边界】`：它不负责什么、不能推导什么，以及与相邻能力的区别。

禁止只写技术名词、实现方式、支持勾叉或“提供某某能力”。技术路径放在`依赖 / App 绑定`，项目支持规则放在`判断依据 / FL 展开条件`。
